	...
	$first=0;
 	...
 	#die "$pieces[48]\n";
		...
		if ($hash{$pieces[48]}{'D'}){
			...
			$overallave{$hash{$pieces[48]}{'D'}}{'ave'}+=$average;
			$overallave{$hash{$pieces[48]}{'D'}}{'C'}=$hash{$pieces[48]}{'C'};
			#die "$pieces[48] $hash{$pieces[48]}{'D'}\n" if ($pieces[48] eq "K10944");
		} else {
...
foreach $levelA (sort keys %overallave){
	print "$levelA\t$overallave{$levelA}{'ave'}\t$overallave{$levelA}{'C'}\n";
}
